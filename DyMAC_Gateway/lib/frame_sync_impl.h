    ...
    class frame_sync_impl : public frame_sync
    {
     private:
        ...
        uint8_t m_freqIndex;
         ...
         std::vector<gr_complex> symb_corr_all;//自己定义
     ...
     public:
      frame_sync_impl(float samp_rate, uint32_t bandwidth, uint8_t sf, bool impl_head, std::vector<uint16_t> sync_word,uint8_t  freqIndex);
      ~frame_sync_impl();

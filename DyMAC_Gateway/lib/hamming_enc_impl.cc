...
namespace gr
{
  namespace lora_sdr
  {
                         ...
                         gr::io_signature::make(1, 1, sizeof(uint8_t)))
    {
      ...
      set_msg_handler(m_inSFPort,boost::bind(&hamming_enc_impl::setSF,this,_1));
    }
    ...
    void
    hamming_enc_impl::setSF(pmt::pmt_t msg){
      // std::cout<<"hamming_enc_impl::parse sf"<<std::endl;
      pmt::pmt_t key = pmt::intern("sf");
      pmt::pmt_t sfNum = pmt::dict_ref(msg,key,pmt::PMT_NIL);
      uint32_t sf = pmt::to_uint64(sfNum);
      m_sf = sf;
      // std::cout<<"hamming_enc_impl::parse sf ending"<<std::endl;
    }

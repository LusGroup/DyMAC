...
namespace gr {
  namespace lora_sdr {
    ...
    frame_sync::sptr
    frame_sync::make(float samp_rate, uint32_t bandwidth, uint8_t sf, bool impl_head, std::vector<uint16_t> sync_word,uint8_t freqIndex)
    {
      return gnuradio::get_initial_sptr
        (new frame_sync_impl(samp_rate, bandwidth, sf, impl_head, sync_word,freqIndex));
    }

    /*
     * The private constructor
     */
    frame_sync_impl::frame_sync_impl(float samp_rate, uint32_t bandwidth, uint8_t sf, bool impl_head, std::vector<uint16_t> sync_word,uint8_t freqIndex)
      : gr::block("frame_sync",
      gr::io_signature::make(1, 1, sizeof(gr_complex)),
      gr::io_signature::make(0, 1, (1u << sf)*sizeof(gr_complex)))//(1u << sf)*sizeof(gr_complex)
      {
        m_state             = DETECT;
        m_bw                = bandwidth;
        ...
        set_msg_handler(m_inSFPort,boost::bind(&frame_sync_impl::setSF,this,_1));
        ...
        preamb_file.open("../../matlab/preambles/preamb.txt", std::ios::out | std::ios::trunc );
        payload_file.open("../../matlab/data/data.txt", std::ios::out | std::ios::trunc );
        #endif
    }
    ...
    void
    frame_sync_impl::setSF(pmt::pmt_t msg){
